...
```
Before refresh:
  User: foobar-user, Org ID: 1111111, Account: 2222222

After refresh:
  User: foobar-user (SAME!), Org ID: 3333333 (NEW!), Account: 4444444 (NEW!)

Result: User creation fails with IntegrityError on username UNIQUE constraint
```
...
3. **If enabled (Stage)**:
   - Rename old user to `old-{username}-{UTC timestamp}` (e.g. `old-test-user-20260615T110000`)
   - Timestamp suffix guarantees uniqueness across multiple Stage refreshes
...
1. **`test_auto_heal_renames_old_user`**
   Flag ON + POST → old user renamed to `old-{username}-{timestamp}`
...
5. **`test_timestamp_suffix_avoids_collisions`**
   Timestamp suffix guarantees uniqueness even if `old-{username}` already exists
