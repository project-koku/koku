...
| Default (out of the box) | **4** calendar months |
| Allowed range | **Minimum 3** / **Maximum 120** months (10 years) |
| Unit | Full calendar months (not N × 30 days) |
| When old data is removed | A scheduled monthly job removes data older than the retention window. Shortening retention does **not** delete data in the API/UI request — eligible data is removed on the **next** scheduled run |
...
The current behavior uses **4** when nothing is configured. Document **4** as the customer-facing default.
