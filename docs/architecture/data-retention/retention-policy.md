...
| **G. Tag value index** | `reporting_ocptags_values` | Same orphan cleanup after provider purge  | No provider FK; `cleanup_ocp_tags_values()` also runs from `delete_archived_data` on source delete |
...
**Source delete:** `delete_source` → `Provider._cascade_delete()` → `post_delete` → `delete_archived_data` → (on-prem) `delete_self_hosted_data_by_source()` / (SaaS) Trino Hive partition drop → `cleanup_ocp_tags_values()` (class G, OCP only). Class E (`rates_to_usage`) still lacks source-delete cleanup (COST-7736).
