...
> **UI TEAM ACTION REQUIRED**: These endpoints use **group-level workspace scoping** instead of RBAC v1's role-level `resourceDefinitions`. While the endpoints are new, the UI building blocks are not — insights-rbac-ui's `CostResources.tsx` and `EditResourceDefinitionsModal.tsx` already implement resource type fetching, instance listing, and multi-select/dual-list picker patterns. The UI team adapts these existing components for the group detail view (see [insights-rbac-ui Extraction](#insights-rbac-ui-extraction) for details).
...
>
> The resource assignment feature uses **group-level workspace scoping** (not role-level `resourceDefinitions`). While insights-rbac v1 supports per-resource scoping via `resourceDefinitions` on custom roles, the on-prem model scopes resources at the group level via Kessel workspaces. This keeps SpiceDB as the single source of truth with no PostgreSQL metadata store.
>
> The following PatternFly components are needed for the group detail view:
>
> 1. **Resource assignment tab** — lists resources currently assigned to the group (`GET /groups/{uuid}/resources/`), with a revoke action per resource (`DELETE /groups/{uuid}/resources/{type}/{id}/`).
...
>
> **Reusable code from insights-rbac-ui**: The existing `CostResources.tsx` (role wizard resource picker) and `EditResourceDefinitionsModal.tsx` (dual-list resource editor) already implement the core patterns needed:
>
> - **Data fetching**: `useResourceTypesQuery` and `useResourceQuery` in `src/shared/data/queries/cost.ts` fetch resource types and instances from Koku's `/api/cost-management/v1/resource-types/` API. This logic is reusable — the on-prem resource picker needs the same data.
> - **Multi-select picker**: `CostResources.tsx` renders a per-permission multi-select with search, select-all, and badge counts. The same component pattern applies to the group-level resource picker, but wired to the bridge's `/groups/{uuid}/resources/` endpoints instead of role `resourceDefinitions`.
> - **Dual-list transfer**: `EditResourceDefinitionsModal.tsx` uses `@data-driven-forms` `DualListSelect` to move resources between "available" and "defined" lists. This maps directly to the bridge's `/groups/{uuid}/resources/available/` (left panel) and `/groups/{uuid}/resources/` (right panel).
>
> The UI team extracts the data-fetching hooks and picker components from the role wizard flow and rewires them into the group detail view. The bridge's REST API response shapes follow the same pagination envelope (`meta.count`, `links.*`, `data[]`) as other bridge endpoints for compatibility.
>
> **Suggested placement**: A "Resources" tab alongside the existing "Members" and "Roles" tabs in the Group detail view.
>
> **Why not reuse role-level `resourceDefinitions` directly?** Role-level scoping requires a metadata store (PostgreSQL) to hold `resourceDefinitions` on roles, since SpiceDB cannot store this. The on-prem design requires SpiceDB/Kessel as the single source of truth with no additional database. Group-level workspace scoping achieves the same access control outcome using native Kessel primitives (`t_workspace` tuples) without any metadata store.
