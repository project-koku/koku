...
DATACATALOG_STORAGE_DESCRIPTOR_SCHEMA = StructType(
    [
        ...
        StructField(
            "sortColumns",
            ArrayType(
                StructType([StructField("column", StringType(), True), StructField("order", IntegerType(), True)]),
                True,
            ),
            True,
        ),
...
class HiveMetastoreTransformer:
    ...
    def transform_skewed_info(self, ms_skewed_col_names, ms_skewed_string_list_values, ms_skewed_col_value_loc_map):
        ...
        return skewed_column_names.join(other=skewed_column_value_location_maps, on="SD_ID", how="outer").join(
    ...
    def transform_ms_serde_info(self, ms_serdes, ms_serde_params):
        ...
        return serde_with_params.rename_columns(rename_tuples=[("NAME", "name"), ("SLIB", "serializationLibrary")])
    ...
    def transform_storage_descriptors(
        ...
        storage_descriptors_joined = (
            ...
            .join_other_to_single_column(
                other=serde_info, on="SERDE_ID", how="left_outer", new_column_name="serdeInfo"
            )
        ...
        return storage_descriptors_with_empty_sorted_cols.drop_columns(["SERDE_ID", "CD_ID"])
    ...
    def transform_databases(self, ms_dbs, ms_database_params):
        ...
        return dbs_drop_invalid.select(struct(dbs_dropped_cols.columns).alias("item")).withColumn(
            "type", lit("database")
        )
...
class DataCatalogTransformer:
    ...
    def generate_idx_for_df(df, id_name, col_name, col_schema):
        ...
        idx_udf = UserDefinedFunction(
            DataCatalogTransformer.udf_array_to_map, MapType(IntegerType(), col_schema, True)
        )
...
def etl_to_metastore(sc, sql_context, hive_metastore, options):
    ...
    databases = sql_context.read.json(path=f"{input_path}databases", schema=METASTORE_DATABASE_SCHEMA)
    tables = sql_context.read.json(path=f"{input_path}tables", schema=METASTORE_TABLE_SCHEMA)
    partitions = sql_context.read.json(path=f"{input_path}partitions", schema=METASTORE_PARTITION_SCHEMA)
...
def transform_df_to_catalog_import_schema(sql_context, glue_context, df_databases, df_tables, df_partitions):
    ...
    df_tables_array = df_tables.select(
        df_tables["type"], df_tables["database"], array(df_tables["item"]).alias("items")
    )
    df_partitions_array_batched = batch_metastore_partitions(sql_context=sql_context, df_parts=df_partitions)
    dyf_databases = DynamicFrame.fromDF(dataframe=df_databases_array, glue_ctx=glue_context, name="dyf_databases")
    dyf_tables = DynamicFrame.fromDF(dataframe=df_tables_array, glue_ctx=glue_context, name="dyf_tables")
...
def metastore_full_migration(
    sc, sql_context, glue_context, connection, datacatalog_name, db_prefix, table_prefix, region
):
    # extract
...
def main():
    ...
    parser.add_argument(
        "-c", "--connection-name", required=False, help="Glue Connection name for Hive metastore JDBC connection"
    )
    parser.add_argument(
        "-R", "--region", required=False, help='AWS region of target Glue DataCatalog, default to "us-east-1"'
    )
    parser.add_argument(
        "-d", "--database-prefix", required=False, help="Optional prefix for database names in Glue DataCatalog"
    )
    ...
    parser.add_argument(
        "-T", "--table-input-path", required=False, help="An S3 path containing json files of metastore table entities"
    )
    parser.add_argument(
        "-P",
