#!/usr/bin/env python3
"""Parse Kibana ES export and extract ingress payload URLs from listener logs.

Input:  kibana-downloading-payload-export.json (Kibana Dev Tools export)
Output: manifest.csv (all unique request_ids)
        manifest-midnight-only.csv (size >= 100KB, kafka timestamp 00:00-02:00 UTC)
        urls.tsv (request_id<TAB>url) for batch download

Usage:
    python parse_kibana_ingress_payload_logs.py
    python parse_kibana_ingress_payload_logs.py --input /path/to/export.json --output-dir ./out
"""
