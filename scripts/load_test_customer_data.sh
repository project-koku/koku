...
if [[ -f "$NISE_REPO_PATH/examples/ocp_on_aws/aws_static_data.yml.example" ]]; then
    rm "$NISE_REPO_PATH/examples/ocp_on_aws/aws_static_data.yml.example"
fi
if [[ -f "$NISE_REPO_PATH/examples/ocp_on_aws/ocp_static_data.yml.example" ]]; then
    rm "$NISE_REPO_PATH/examples/ocp_on_aws/ocp_static_data.yml.example"
fi
if [[ -f "$NISE_REPO_PATH/examples/ocp_on_azure/azure_static_data.yml.example" ]]; then
    rm "$NISE_REPO_PATH/examples/ocp_on_azure/azure_static_data.yml.example"
fi
if [[ -f "$NISE_REPO_PATH/examples/ocp_on_azure/ocp_static_data.yml.example" ]]; then
    rm "$NISE_REPO_PATH/examples/ocp_on_azure/ocp_static_data.yml.example"
fi
...
COST_MODEL_JSON=$(cat "$KOKU_REPO/scripts/openshift_on_prem_cost_model.json" | sed -e "s/PROVIDER_UUID/$OCP_ON_PREM_UUID/g")

curl --header "Content-Type: application/json" \
  --request POST \
  --data "$COST_MODEL_JSON" \
  http://localhost:8000$API_PATH_PREFIX/v1/costmodels/
...
curl http://localhost:5000$API_PATH_PREFIX/v1/download/
