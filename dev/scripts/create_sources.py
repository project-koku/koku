...
KOKU_SOURCES_URL = (
    f"http://{KOKU_API_HOST}:{KOKU_API_PORT}{KOKU_API_PATH_PREFIX}/v1/sources"
)
...
def create_parser():
    ...
    parser.add_argument(
        "--role_arn", dest="role_arn", required=False, help="AWS roleARN"
    )
    parser.add_argument(
        "--source_id",
        dest="source_id",
    ...
    provider_group.add_argument(
        "--aws", dest="aws", action="store_true", help="Create an AWS source."
    )
...
class SourcesClientDataGenerator:
    ...
    def create_azure_storage(
        self, parameters, resource_group, storage_account, scope=None, export_name=None
    ):
        json_data = {
            "billing_source": {
                "data_source": {
...
class SourcesDataGenerator:
    ...
    def create_source(self, source_name, source_type, cluster_id=None):
        json_data = {
            "source_type_id": str(self._source_type_map.get(source_type)),
            "name": source_name,
        }
        if cluster_id:
...
def main():  # noqa
    ...
    if app_create_source_id:
        application_id = generator.create_application(
            app_create_source_id, "/insights/platform/cost-management"
        )
        print(
            f"Attached Cost Management Application ID {application_id} to Source ID {app_create_source_id}"
        )
        return
    ...
    if parameters.get("aws"):
        ...
        if s3_bucket and source_id_param:
            ...
            billing_source_response = sources_client.create_s3_bucket(
                parameters, s3_bucket, s3_region
            )
        ...
        authentication_id = generator.create_aws_authentication(
            endpoint_id, "user@example.com", role_arn
        )
        ...
        if create_application:
            application_id = generator.create_application(
                source_id, "/insights/platform/cost-management"
            )
            print(
                f"Attached Cost Management Application ID {application_id} to Source ID {source_id}"
            )
