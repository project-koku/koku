...
class OCPQuotasView(generics.ListAPIView):
    """API GET list view for OpenShift quotas."""

    queryset = (
        OCPCostSummaryByQuotaP.objects.annotate(**{"value": F("quota_name")})
        .values("value")
        .distinct()
        .filter(quota_name__isnull=False)
    )
