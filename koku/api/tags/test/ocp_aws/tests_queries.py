...
class OCPAWSTagQueryHandlerTest(IamTestCase):
    ...
    def test_1_month_only_keys(self):
        """Test that execute_query() succeeds with 1 month parameters, keys-only."""
        url = (
            "?filter[time_scope_units]=month&filter[time_scope_value]=-1&filter[resolution]=monthly&key_only=True"
        )  # noqa: E501
        query_params = self.mocked_query_params(url, OCPAWSTagView)
        handler = OCPAWSTagQueryHandler(query_params)
        query_output = handler.execute_query()
