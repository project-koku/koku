...
class ProviderMap(object):
    ...
    ProviderMap supplies OCPReportQueryHandler with the appropriate model
    references.
    """
...
class OCPReportQueryHandler(object):
    ...
    def order_field(self):
        ...
        order_by = self.query_parameters.get('order_by', default_ordering)
    ...
    def _get_search_filter(self, filters):
        ...
        for q_param, filt in fields.items():
            ...
            if list_ and not OCPReportQueryHandler.has_wildcard(list_):
                for item in list_:
    ...
    def _group_data_by_list(group_by_list, group_index, data):
        ...
        for key, group in groupby(data, lambda by: by.get(curr_group)):
            grouped = list(group)
            grouped = OCPReportQueryHandler._group_data_by_list(group_by_list,
                                                             (group_index + 1),
                                                             grouped)
    ...
    def _apply_group_by(self, query_data):
        ...
        for date, data_list in bucket_by_date.items():
            ...
            group_by = self._get_group_by()
            grouped = OCPReportQueryHandler._group_data_by_list(group_by, 0,
                                                             data)
