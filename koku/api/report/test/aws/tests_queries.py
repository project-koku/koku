...
class AWSReportQueryTest(IamTestCase):
    ...
    def test_prefixed_logical_and(self):
        ...
        account_ab_fake_aws = FakeAWSCostData(self.provider, account_alias='_APPLESAUCE_BANANA')
        ...
        account_ac_fake_aws = FakeAWSCostData(self.provider, account_alias='_APPLESAUCE_CANNONS')
        ...
        # Query 1 - a AND b
        query_1_url = "?group_by[and:account]=_APPLESAUCE&group_by[and:account]=_BANANA&filter[time_scope_value]=-1&filter[time_scope_units]=month"  # noqa
        ...
        # Query 2 - ab
        query_2_url = "?group_by[account]=_APPLESAUCE_BANANA&filter[time_scope_value]=-1&filter[time_scope_units]=month"  # noqa
        ...
        # We never added any data that would have matched this query, so the result should be 0
        query_3_url = "?group_by[and:account]=_APPLESAUCE&group_by[and:account]=_BANANA&group_by[and:account]=_CANNONS&filter[time_scope_value]=-1&filter[time_scope_units]=month"  # noqa
        ...
        query_4_url = "?group_by[account]=_APPLESAUCE&group_by[account]=_BANANA&filter[time_scope_value]=-1&filter[time_scope_units]=month"  # noqa
