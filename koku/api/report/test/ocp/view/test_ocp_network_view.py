...
class OCPReportViewNetworkTest(IamTestCase):
    ...
    def setUp(self) -> None:
        ...
        with schema_context(self.schema_name):
            self.total_in = sum(
                OCPNetworkSummaryP.objects
                .filter(usage_start__gte=self.ten_days_ago.date(), infrastructure_data_in_gigabytes__isnull=False)
                .values_list("infrastructure_data_in_gigabytes", flat=True)
            )
            self.total_out = sum(
                OCPNetworkSummaryP.objects
                .filter(usage_start__gte=self.ten_days_ago.date(), infrastructure_data_out_gigabytes__isnull=False)
    ...
    def test_get_node_network_costs_group_by(self):
        with schema_context(self.schema_name):
            params = {
                "group_by[project]": "*",
            }
