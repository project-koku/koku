...
class OCPReportQueryHandlerTest(IamTestCase):
    ...
    def test_line_item_wasted_cost_corrects_pooling_bug(self):
        """_line_item_wasted_cost uses per-pod daily grain, not cluster-bucket aggregates.

        Two pods in the same cluster/day pool to 100% utilisation at bucket level,
        driving the naive bucket-level waste formula to zero. But per-pod, pod-A is
        only 50% utilised and carries real waste. This test confirms the method
        returns the per-pod sum (50) rather than the bucket result (0).

        Cost basis: infrastructure_raw_cost (OCP-on-cloud path, no cost model).
        effective_usage = SUM(MAX(usage_i, request_i)) per interval — for a single-interval
        pod this equals MAX(usage, request).  No cost model → rate = 0, so:
          per_row_cost = effective × 0 + infra_raw × 1.0 = 100.
        Pod A: waste = 100 × (1 − usage/effective) = 100 × (1 − 5/10) = 50.
        Pod B: waste = max(0, 100 × (1 − 15/15)) = 0.
        """
        usage_date = self.dh.yesterday.date()
        cluster_id = "pooling-test-cluster-waste"

        with tenant_context(self.tenant):
            OCPUsageLineItemDailySummary.objects.filter(cluster_id=cluster_id).delete()

            # Pod A: 50% utilized → effective=MAX(5,10)=10, waste = 100 × (1 − 5/10) = 50
            self.baker.make(
                OCPUsageLineItemDailySummary,
                cluster_id=cluster_id,
                data_source="Pod",
                namespace="pooling-ns",
                usage_start=usage_date,
                usage_end=usage_date,
                pod_request_cpu_core_hours=Decimal("10"),
                pod_usage_cpu_core_hours=Decimal("5"),
                pod_effective_usage_cpu_core_hours=Decimal("10"),
                infrastructure_raw_cost=Decimal("100"),
                infrastructure_markup_cost=Decimal("0"),
                raw_currency="USD",
                cost_model_rate_type=None,
            )
            # Pod B: 150% utilized → effective=MAX(15,10)=15, waste = max(0, 100×(1−15/15)) = 0
            self.baker.make(
                OCPUsageLineItemDailySummary,
                cluster_id=cluster_id,
                data_source="Pod",
                ...
                pod_effective_usage_cpu_core_hours=Decimal("15"),
                infrastructure_raw_cost=Decimal("100"),
                infrastructure_markup_cost=Decimal("0"),
                raw_currency="USD",
                cost_model_rate_type=None,
            ...
            url = f"?filter[cluster]={cluster_id}"
            query_params = self.mocked_query_params(url, OCPCpuView)
            handler = OCPReportQueryHandler(query_params)
            query_group_by = ["date"]

            waste_map = handler._line_item_wasted_cost(query_group_by)

        # Per-pod sum: 50 + 0 = 50.  Bucket-level would be 200 × (1 − 20/20) = 0.
        self.assertEqual(waste_map["__total__"], Decimal("50"))

    def test_line_item_wasted_cost_memory_corrects_pooling_bug(self):
        """Same pooling-bug correction for the memory report variant.

        Cost basis: infrastructure_raw_cost (OCP-on-cloud path, no cost model).
        effective_usage = SUM(MAX(usage_i, request_i)) per interval.  No cost model → rate = 0, so:
          per_row_cost = effective × 0 + infra_raw × infra_exchange_rate = 100.
        Pod A: waste = 100 × (1 − 5/10) = 50.  Pod B: waste = max(0, 100×(1 − 15/15)) = 0.
        """
        usage_date = self.dh.yesterday.date()
        cluster_id = "pooling-test-cluster-waste-mem"
        ...
        with tenant_context(self.tenant):
            ...
            self.baker.make(
                ...
                pod_effective_usage_memory_gigabyte_hours=Decimal("15"),
                infrastructure_raw_cost=Decimal("100"),
                infrastructure_markup_cost=Decimal("0"),
                raw_currency="USD",
                cost_model_rate_type=None,
