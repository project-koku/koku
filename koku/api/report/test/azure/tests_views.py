...
class AzureReportViewTest(IamTestCase):
    ...
    def test_process_invalid_query_parameters_format(self):
        ...
        qs = 'group_by%5Bsubscription%5D=subscription1&filter%5'
    ...
    def test_process_query_parameters(self):
        ...
        qs = 'group_by%5Bsubscription%5D=subscription1&filter%5Bresolution%5D=daily'
        valid, query_dict = process_query_parameters(qs, AzureQueryParamSerializer)
        self.assertTrue(valid)
        self.assertEqual(query_dict.get('group_by'), {'subscription': ['subscription1']})
    ...
    def test_process_query_parameters_invalid(self):
        ...
        qs = 'group_by%5Binvalid%5D=subscription1&filter%5Bresolution%5D=daily'
    ...
    def test_convert_units_total_not_dict(self):
        ...
        report = self.report['data'][0]['subscriptions'][0]['values'][0]
    ...
    def test_execute_query_w_delta_bad_choice(self):
        ...
        qs = f'group_by[subscription]=*&filter[limit]=2&delta={bad_delta}'
        url = reverse('reports-azure-costs') + '?' + qs
        client = APIClient()
        response = client.get(url, **self.headers)
