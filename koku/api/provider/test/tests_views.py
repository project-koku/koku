...
class ProviderViewTest(IamTestCase):
    ...
    def create_provider(self, bucket_name, iam_arn, headers=None, provider_type=Provider.PROVIDER_AWS):
    ...
    def test_create_aws_with_no_provider_resource_name(self):
        ...
        provider = {
            ...
            "authentication": {"provider_resource_name": ""},
    ...
    def test_create_provider(self):
        ...
        self.assertEqual(json_result.get("customer").get("account_id"), self.customer_data.get("account_id"))
        self.assertIsNotNone(json_result.get("created_by"))
        self.assertEqual(json_result.get("created_by").get("username"), self.user_data.get("username"))
    ...
    def test_list_provider(self):
        ...
        request_context = self._create_request_context(
            self.create_mock_customer_data(), self._create_user_data(), create_tenant=True
        )
        ...
        expected_cost_model_info = {"name": cost_model_result["name"], "uuid": cost_model_result["uuid"]}

        # Create a second provider but for a different user.
        iam_arn2 = "arn:aws:s3:::a_s3_bucket"
        bucket_name2 = "a_s3_bucket"
        second_create_response = self.create_provider(bucket_name2, iam_arn2, alternate_headers)
    ...
    def test_remove_provider_with_regular_user(self):
        ...
        self.assertEqual(json_result.get("customer").get("account_id"), self.customer_data.get("account_id"))
        self.assertIsNotNone(json_result.get("created_by"))
        self.assertEqual(json_result.get("created_by").get("username"), self.user_data["username"])
