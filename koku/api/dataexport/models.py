...
class DataExportRequest(models.Model):
    ...
    def __repr__(self):
        ...
        start_date = repr(self.start_date.isoformat()) if self.start_date is not None else None
        end_date = repr(self.end_date.isoformat()) if self.end_date is not None else None
        try:
            created_by = self.created_by.id
        except User.DoesNotExist:
            created_by = None
        created_timestamp = repr(self.created_timestamp.isoformat()) if self.created_timestamp is not None else None
        updated_timestamp = repr(self.updated_timestamp.isoformat()) if self.updated_timestamp is not None else None
        return (
