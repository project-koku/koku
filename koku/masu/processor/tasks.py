...
def get_report_files(  # noqa: C901
    ...
    try:
        ...
        try:
            ...
            LOG.error(log_json(tracing_id, f"Report processing error: {str(processing_error)}", context))
            WorkerCache().remove_task_from_cache(cache_key)
            raise processing_error
        except NotImplementedError as err:
            LOG.info(log_json(tracing_id, f"Not implemented error: {str(err)}", context))
            WorkerCache().remove_task_from_cache(cache_key)
        ...
        LOG.warning(log_json(tracing_id, f"Report downloader Warning: {str(err)}", context))
        WorkerCache().remove_task_from_cache(cache_key)
    except Exception as err:
        worker_stats.PROCESS_REPORT_ERROR_COUNTER.labels(provider_type=provider_type).inc()
        LOG.error(log_json(tracing_id, f"Unknown downloader exception: {str(err)}", context))
        WorkerCache().remove_task_from_cache(cache_key)
