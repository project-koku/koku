...
def update_summary_tables(  # noqa: C901
    ...
    if ocp_on_cloud and not is_managed_ocp_cloud_summary_enabled(schema):
...
def process_openshift_on_cloud_trino(
    ...
    for report in reports_deduplicated:
        schema_name = report.get("schema_name")
        context = {"provider_type": provider_type, "schema_name": schema_name, "provider_uuid": provider_uuid}
        if provider_type not in Provider.MANAGED_OPENSHIFT_ON_CLOUD_PROVIDER_LIST:
            LOG.info(
                log_json(
                    tracing_id, msg="provider type not valid for COST-5129 OCP on cloud processing", context=context
                )
            )
            continue
        if not is_managed_ocp_cloud_processing_enabled(schema_name):
            LOG.info(
                log_json(tracing_id, msg="provider not enabled for COST-5129 OCP on cloud processing", context=context)
            )
            continue
        with ReportManifestDBAccessor() as manifest_accesor:
            tracing_id = report.get("tracing_id", report.get("manifest_uuid", "no-tracing-id"))
        ...
        for month in months:
            ...
            manifest_id = report.get("manifest_id")
            processor = OCPCloudParquetReportProcessor(
                schema_name, "", provider_uuid, provider_type, manifest_id, context
            )
            processor.process_ocp_cloud_trino(start_date, end_date)
            if is_managed_ocp_cloud_summary_enabled(schema_name):
                LOG.info("HEY LOOK MA I MADE IT AGAIN")
                trigger_ocp_on_cloud_summary(
                    context, schema_name, provider_uuid, manifest_id, tracing_id, start_date, end_date
                )
