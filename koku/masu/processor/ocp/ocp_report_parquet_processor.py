...
class OCPReportParquetProcessor(ReportParquetProcessorBase):
    ...
    def create_bill(self, bill_date):
        ...
        report_period_start = ciso8601.parse_datetime(start_date).replace(hour=0, minute=0, tzinfo=ZoneInfo("UTC"))
        report_period_end = ciso8601.parse_datetime(end_date).replace(hour=0, minute=0, tzinfo=ZoneInfo("UTC"))
