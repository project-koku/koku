...
def copy_data_to_s3_bucket(request_id, path, filename, data, manifest_id=None, context={}):
    ...
    """
    LOG.info(f"copy_data_to_s3_bucket context: {str(context)}")
    if not (settings.ENABLE_S3_ARCHIVING or enable_trino_processing(context.get("provider_uuid"), context.get("provider_type"), context.get("account"))):
        return None
...
def copy_local_report_file_to_s3_bucket(
    ...
    """
    LOG.info(f"copy_local_report_file_to_s3_bucket context: {str(context)}")
    if s3_path and (settings.ENABLE_S3_ARCHIVING or enable_trino_processing(context.get("provider_uuid"), context.get("provider_type"), context.get("account"))):
