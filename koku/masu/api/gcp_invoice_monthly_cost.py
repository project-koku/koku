...
def gcp_invoice_monthly_cost(request):
    ...
    account_info = AccountsAccessor().get_accounts(provider_uuid)[0]
    project_id = account_info.get("credentials", {}).get("project_id")
    dataset = account_info.get("data_source", {}).get("dataset")
    table_id = account_info.get("data_source", {}).get("table_id")
