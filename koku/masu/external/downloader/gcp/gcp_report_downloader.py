...
class GCPReportDownloader(ReportDownloaderBase, DownloaderInterface):
    ...
    def get_report_context_for_date(self, date_time):
        ...
        if not self.check_if_manifest_should_be_downloaded(
    ...
    def download_file(self, key, stored_etag=None):
        ...
        if not blob:
            raise GCPReportDownloaderNoFileError(
                f'No blob found in bucket "{self.bucket_name}" with name "{key}"'
            )
    ...
    def _get_local_directory_path(self):
        ...
        directory_path = os.path.join(
            DATA_DIR, safe_customer_name, 'gcp', safe_bucket_name
        )
