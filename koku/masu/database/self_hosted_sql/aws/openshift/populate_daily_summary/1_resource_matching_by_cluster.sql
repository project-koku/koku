...
cte_matchable_resource_names AS (
    ...
    JOIN cte_array_agg_nodes AS nodes
        ON right(resource_names.lineitem_resourceid, length(nodes.resource_id)) = nodes.resource_id
    ...
    JOIN cte_array_agg_volumes AS volumes
        ON (
            right(resource_names.lineitem_resourceid, length(volumes.persistentvolume)) = volumes.persistentvolume
            OR (volumes.csi_volume_handle != '' AND right(resource_names.lineitem_resourceid, length(volumes.csi_volume_handle)) = volumes.csi_volume_handle)
        )
),
cte_agg_tags AS (
