...
WITH cte_hours as (
    SELECT EXTRACT(DAY FROM (DATE_TRUNC('month', {{start_date}}) + INTERVAL '1 month - 1 day')) * 24 as in_month
),
...
SELECT  uuid_generate_v4()::text as row_uuid, -- need a new uuid or it will deduplicate
    ...
    CASE
        WHEN max(ocp.persistentvolumeclaim) = ''
            THEN NULL::jsonb
        ELSE ocp.pod_labels::jsonb
    END as pod_labels,
    CASE
        WHEN max(ocp.persistentvolumeclaim) = ''
            THEN NULL::jsonb
        ELSE ocp.volume_labels::jsonb
...
SELECT  uuid_generate_v4()::text as row_uuid, -- need a new uuid or it will deduplicate
    ...
    max(aws.region) as region,
    max(aws.unit) as unit,
    cast(NULL as double precision) as usage_amount,
    max(aws.currency_code) as currency_code,
...
SELECT aws.row_uuid,
        ...
        ocp.pod_labels::jsonb as pod_labels,
        max(aws.tags) as tags,
...
SELECT aws.row_uuid,
        ...
        max(aws.calculated_amortized_cost) as calculated_amortized_cost,
        max(aws.calculated_amortized_cost) * cast({{markup}} as decimal(33,9)) as markup_cost_amortized,
        max(ocp.pod_labels)::jsonb as pod_labels,
        max(ocp.volume_labels)::jsonb as volume_labels,
        max(aws.tags) as tags,
        max(aws.aws_cost_category) as aws_cost_category,
        max(ocp.cost_category_id) as cost_category_id,
    ...
    JOIN {{schema | sqlsafe}}.managed_aws_openshift_daily_temp as aws
        ON aws.usage_start = ocp.usage_start
            AND (
                aws.tags::jsonb->>'openshift_project' = ocp.namespace
                OR aws.tags::jsonb->>'openshift_node' = ocp.node
                OR aws.tags::jsonb->>'openshift_cluster' = ocp.cluster_alias
                OR aws.tags::jsonb->>'openshift_cluster' = ocp.cluster_id
                OR (aws.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(aws.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.pod_labels, ' ', ''), replace(tag, ' ', '')) != 0))
                OR (aws.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(aws.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.volume_labels, ' ', ''), replace(tag, ' ', '')) != 0))
            )
...
WITH cte_cluster_counts AS (
    ...
    JOIN {{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary_staging as ocp
        ON aws_temp.usage_start = ocp.usage_start
            AND (
                aws_temp.tags::jsonb->>'openshift_project' = ocp.namespace
                OR aws_temp.tags::jsonb->>'openshift_node' = ocp.node
                OR aws_temp.tags::jsonb->>'openshift_cluster' = ocp.cluster_alias
                OR aws_temp.tags::jsonb->>'openshift_cluster' = ocp.cluster_id
                OR (aws_temp.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(aws_temp.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.pod_labels, ' ', ''), replace(tag, ' ', '')) != 0))
                OR (aws_temp.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(aws_temp.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.volume_labels, ' ', ''), replace(tag, ' ', '')) != 0))
            )
