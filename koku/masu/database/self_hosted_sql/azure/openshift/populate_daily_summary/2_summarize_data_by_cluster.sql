...
SELECT uuid_generate_v4()::text as row_uuid, -- need a new uuid or it will deduplicate
    ...
    WHERE ocp.source = {{ocp_provider_uuid}}
        ...
        AND azure.resource_id_matched = True
        AND ocp.namespace != 'Storage unattributed'
        AND az_disk.capacity != pv_cap.total_pv_capacity -- prevent inserting zero cost rows
        AND az_disk.year = {{year}}
        AND az_disk.month = {{month}}
...
SELECT azure.row_uuid,
    ...
    JOIN {{schema | sqlsafe}}.managed_azure_openshift_daily_temp as azure
        ON azure.usage_start = ocp.usage_start
        AND (
                azure.tags::jsonb->>'openshift_project' = ocp.namespace
                OR azure.tags::jsonb->>'openshift_node' = ocp.node
                OR azure.tags::jsonb->>'openshift_cluster' = ocp.cluster_alias
                OR azure.tags::jsonb->>'openshift_cluster' = ocp.cluster_id
                OR (azure.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(azure.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.pod_labels, ' ', ''), replace(tag, ' ', '')) != 0))
                OR (azure.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(azure.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.volume_labels, ' ', ''), replace(tag, ' ', '')) != 0))
            )
...
WITH cte_cluster_counts AS (
    ...
    SELECT azure_temp.row_uuid,
        count(DISTINCT ocp.source) as cluster_count
    FROM {{schema | sqlsafe}}.managed_azure_openshift_daily_temp AS azure_temp
    JOIN {{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary_staging as ocp
        ON azure_temp.usage_start = ocp.usage_start
            AND (
                azure_temp.tags::jsonb->>'openshift_project' = ocp.namespace
                OR azure_temp.tags::jsonb->>'openshift_node' = ocp.node
                OR azure_temp.tags::jsonb->>'openshift_cluster' = ocp.cluster_alias
                OR azure_temp.tags::jsonb->>'openshift_cluster' = ocp.cluster_id
                OR (azure_temp.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(azure_temp.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.pod_labels, ' ', ''), replace(tag, ' ', '')) != 0))
                OR (azure_temp.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(azure_temp.matched_tag, ',')) AS tag WHERE strpos(replace(ocp.volume_labels, ' ', ''), replace(tag, ' ', '')) != 0))
            )
        AND ocp.namespace != 'Worker unallocated'
        AND ocp.namespace != 'Platform unallocated'
        AND ocp.namespace != 'Storage unattributed'
        AND ocp.namespace != 'Network unattributed'
    WHERE azure_temp.source = {{cloud_provider_uuid}}
        AND azure_temp.year = {{year}}
        AND azure_temp.month = {{month}}
        AND azure_temp.resource_id_matched = FALSE
        AND azure_temp.matched_tag IS NOT NULL
        AND azure_temp.matched_tag != ''
        AND ocp.year = {{year}}
        AND lpad(ocp.month, 2, '0') = {{month}}
        AND ocp.usage_start >= {{start_date}}
        AND ocp.usage_start < {{end_date}} + INTERVAL '1 day'
        -- Don't filter by ocp.source here - we want to count ALL matching OCP sources
    GROUP BY azure_temp.row_uuid
),
cte_rankings AS (
    SELECT pds.row_uuid,
...
SELECT pds.row_uuid,
    ...
    CASE WHEN pds.resource_id_matched = FALSE
        THEN usage_quantity / (r.azure_uuid_count * r.cluster_count)
        ELSE usage_quantity / r.azure_uuid_count
    END as usage_quantity,
    currency,
    CASE WHEN resource_id_matched = TRUE AND data_source = 'Pod'
        THEN ({{pod_column | sqlsafe}} / nullif({{node_column | sqlsafe}}, 0)) * pretax_cost
        WHEN resource_id_matched = FALSE
        THEN pretax_cost / (r.azure_uuid_count * r.cluster_count)
        ELSE pretax_cost / r.azure_uuid_count
    END as pretax_cost,
    CASE WHEN resource_id_matched = TRUE AND data_source = 'Pod'
        THEN ({{pod_column | sqlsafe}} / nullif({{node_column | sqlsafe}}, 0)) * pretax_cost * cast({{markup}} as decimal(24,9))
        WHEN resource_id_matched = FALSE
        THEN pretax_cost / (r.azure_uuid_count * r.cluster_count) * cast({{markup}} as decimal(24,9))
