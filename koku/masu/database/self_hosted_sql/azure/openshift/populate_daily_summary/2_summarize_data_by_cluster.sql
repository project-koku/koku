    ...
    JOIN {{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary_staging as ocp
        ON azure_temp.usage_start = ocp.usage_start
            AND (
                ...
                OR (azure_temp.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(azure_temp.matched_tag, ',')) AS tag WHERE strpos(ocp.pod_labels, replace(tag, ' ', '')) != 0))
                OR (azure_temp.matched_tag != '' AND EXISTS (SELECT 1 FROM unnest(string_to_array(azure_temp.matched_tag, ',')) AS tag WHERE strpos(ocp.volume_labels, replace(tag, ' ', '')) != 0))
            )
