...
WITH cte_line_items AS (
    SELECT date(date) as usage_date,
        ...
        (additionalinfo::jsonb->>'ServiceType') as instance_type,
...
SELECT uuid_generate_v4() as uuid,
    ...
    (SELECT json_object_agg(key, value) FROM jsonb_each_text(li.tags::jsonb) WHERE key = ANY(pek.keys))::jsonb as tags,
