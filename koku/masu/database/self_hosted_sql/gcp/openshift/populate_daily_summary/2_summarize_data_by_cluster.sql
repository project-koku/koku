...
DELETE FROM {{schema | sqlsafe}}.managed_gcp_openshift_disk_capacities_temp
...
{% if unattributed_storage %}
INSERT INTO {{schema | sqlsafe}}.managed_gcp_openshift_disk_capacities_temp (
...
SELECT
    ...
    {{year}} as year,
    {{month}} as month
FROM {{schema | sqlsafe}}.gcp_line_items as gcp
WHERE gcp.resource_global_name in (
        SELECT temp.resource_global_name as resource_global_name
        FROM {{schema | sqlsafe}}.managed_gcp_openshift_daily_temp as temp
        INNER JOIN {{schema | sqlsafe}}.openshift_storage_usage_line_items_daily as ocp
...
JOIN {{schema | sqlsafe}}.managed_gcp_openshift_disk_capacities_temp as gcp_disk
    ...
    ON gcp.usage_start = ocp.usage_start
        AND (
                ...
                OR (gcp.matched_tag != '' AND EXISTS (
                    ...
                    WHERE strpos(replace(ocp.pod_labels, ' ', ''), replace(tag, ' ', '')) != 0
                ))
                OR (gcp.matched_tag != '' AND EXISTS (
                    SELECT 1 FROM unnest(string_to_array(gcp.matched_tag, ',')) AS tag
                    WHERE strpos(replace(ocp.volume_labels, ' ', ''), replace(tag, ' ', '')) != 0
                ))
            )
