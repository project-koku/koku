...
class BaseSQLRunner(metaclass=ABCMeta):
    @property
    @abstractmethod
...
class PostgresSQLRunner(BaseSQLRunner):
    prepare_query = JinjaSql(param_style="pyformat").prepare_query

    @classmethod
    def execute(cls, schema: str, *, sql_params: dict | None = None) -> None:
        sql, params = cls.prepare_query(cls.sql_template(), sql_params)
        ...
        LOG.info(log_json(msg=f"finished {cls.template_name}", running_time=running_time))
