...
cte_distinct_values_agg AS (
    ...
    FROM (
        ...
        LEFT JOIN {{schema_name | sqlsafe}}.reporting_awstags_summary AS ls
            ON va.key = ls.key
...
ins1 AS (
    INSERT INTO {{schema_name | sqlsafe}}.reporting_awstags_summary (uuid, key, cost_entry_bill_id, usage_account_id, account_alias_id, values)
...
INSERT INTO {{schema_name | sqlsafe}}.reporting_awstags_values (uuid, key, value, usage_account_ids, account_aliases)
SELECT uuid_generate_v4() as uuid,
    tv.key,
    tv.value,
...
DELETE FROM {{schema_name | sqlsafe}}.reporting_awstags_summary AS ts
WHERE EXISTS (
    SELECT 1
...
WITH cte_expired_tag_keys AS (
    ...
    FROM {{schema_name | sqlsafe}}.reporting_awstags_values AS tv
    LEFT JOIN {{schema_name | sqlsafe}}.reporting_awstags_summary AS ts
        ON tv.key = ts.key
    WHERE ts.key IS NULL

)
DELETE FROM {{schema_name | sqlsafe}}.reporting_awstags_values tv
