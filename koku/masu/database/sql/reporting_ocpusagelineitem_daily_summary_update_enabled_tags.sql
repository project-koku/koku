...
update {{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary as lids
   set pod_labels = coalesce(nullif(pod_labels, '"{}"')::jsonb, '{}'::jsonb) - array_subtract(array(select jsonb_object_keys(coalesce(nullif(pod_labels, '"{}"')::jsonb, '{}'::jsonb)))::text[], keys::text[]),
       volume_labels = coalesce(nullif(volume_labels, '"{}"')::jsonb, '{}'::jsonb) - array_subtract(array(select jsonb_object_keys(coalesce(nullif(volume_labels, '"{}"')::jsonb, '{}'::jsonb)))::text[], keys::text[])
