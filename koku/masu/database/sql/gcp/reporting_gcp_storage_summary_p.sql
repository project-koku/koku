DELETE FROM {{schema | sqlsafe}}.reporting_gcp_storage_summary_p
...
INSERT INTO {{schema | sqlsafe}}.reporting_gcp_storage_summary_p (
    ...
    WHERE usage_start >= {{start_date}}::date
        AND usage_start <= {{end_date}}::date
        AND source_uuid = {{source_uuid}}
        AND service_alias NOT IN ('Filestore', 'Storage', 'Cloud Storage', 'Data Transfer')
        AND invoice_month = {{invoice_month}}
        AND unit = 'gibibyte month'
    GROUP BY usage_start, source_uuid, invoice_month
;
