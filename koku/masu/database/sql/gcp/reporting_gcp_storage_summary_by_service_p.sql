DELETE FROM {{schema | sqlsafe}}.reporting_gcp_storage_summary_by_service_p
...
INSERT INTO {{schema | sqlsafe}}.reporting_gcp_storage_summary_by_service_p (
    ...
    WHERE usage_start >= {{start_date}}::date
        AND usage_start <= {{end_date}}::date
        AND source_uuid = {{source_uuid}}
        AND (service_alias IN ('Filestore', 'Storage', 'Cloud Storage', 'Data Transfer') OR unit = 'gibibyte month')
        AND invoice_month = {{invoice_month}}
