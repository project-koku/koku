WITH cte_tag_key_mapping AS (
    ...
    WHERE
        ...
        AND (
            enabledtagkeys.provider_type = 'GCP'
...
cte_update_tag_keys as (
    SELECT
        ...
        END as update_tags
    FROM
        {{schema | sqlsafe}}.reporting_ocpgcpcostlineitem_project_daily_summary_p AS lids
    CROSS JOIN (
        SELECT
...
UPDATE {{schema | sqlsafe}}.reporting_ocpgcpcostlineitem_project_daily_summary_p AS lids
