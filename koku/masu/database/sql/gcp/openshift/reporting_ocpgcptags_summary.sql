WITH cte_tag_value AS (
    SELECT key,
        ...
        li.namespace,
        node
    FROM {{schema_name | sqlsafe}}.reporting_ocpgcpcostlineitem_project_daily_summary_p AS li,
...
cte_distinct_values_agg AS (
    ...
    FROM (
        ...
        LEFT JOIN {{schema_name | sqlsafe}}.reporting_ocpgcptags_summary AS ls
            ON va.key = ls.key
...
ins1 AS (
    INSERT INTO {{schema_name | sqlsafe}}.reporting_ocpgcptags_summary (uuid, key, values, cost_entry_bill_id, report_period_id, account_id, project_id, project_name, namespace, node)
...
INSERT INTO {{schema_name | sqlsafe}}.reporting_ocpgcptags_values (uuid, key, value, account_ids, project_ids, project_names, cluster_ids, cluster_aliases, namespaces, nodes)
SELECT uuid_generate_v4() as uuid,
    tv.key,
    tv.value,
...
DELETE FROM {{schema_name | sqlsafe}}.reporting_ocpgcptags_summary AS ts
WHERE EXISTS (
    SELECT 1
...
WITH cte_expired_tag_keys AS (
    ...
    FROM {{schema_name | sqlsafe}}.reporting_ocpgcptags_values AS tv
    LEFT JOIN {{schema_name | sqlsafe}}.reporting_ocpgcptags_summary AS ts
        ON tv.key = ts.key
    WHERE ts.key IS NULL

)
DELETE FROM {{schema_name | sqlsafe}}.reporting_ocpgcptags_values tv
