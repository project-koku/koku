...
SELECT uuid_generate_v4(),
    ...
    sum(coalesce(lids.pod_usage_memory_gigabyte_hours, 0)) * {{memory_gb_usage_per_hour}}
        ...
        + sum(coalesce(lids.pod_effective_usage_memory_gigabyte_hours, 0)) * {{node_core_cost_per_hour}}
        + sum(coalesce(lids.pod_effective_usage_memory_gigabyte_hours, 0)) * {{cluster_core_cost_per_hour}}
        {% endif %}
        + coalesce((
