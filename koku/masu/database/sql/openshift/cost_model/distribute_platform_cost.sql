...
-- Notes:
-- The sql below calculates the platform cost at a node
-- level then adds that cost in as a negative so that
-- the platform cost we distributed to the user's projects
-- is zero'd out.
-- To validate use same SQL as above but change cost_category_id filter
...
WHERE lids.usage_start >= {{start_date}}::date
    ...
    AND cat.name = 'Platform'
    AND lids.source_uuid = {{source_uuid}}
GROUP BY lids.usage_start, lids.cluster_id, lids.source_uuid, lids.node, lids.namespace;
