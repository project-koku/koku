...
CREATE TEMPORARY TABLE node_to_platform_cost AS (
    ...
    WHERE lids.cost_category_id IS NOT NULL
        ...
        AND source_uuid = {{source_uuid}}
    GROUP BY source_uuid, lids.node, lids.cluster_id, lids.namespace, cost_category_id
);
...
WHERE usage_start >= {{start_date}}::date
    ...
    AND source_uuid = {{source_uuid}}
GROUP BY usage_start, lids.node, source_uuid, lids.cluster_id, lids.namespace, pod_labels;
