DELETE FROM {{schema | sqlsafe}}.reporting_ocp_cost_summary_by_quota_p
WHERE usage_start >= {{start_date}}::date
    AND usage_start <= {{end_date}}::date
    AND source_uuid = {{source_uuid}}
;

INSERT INTO {{schema | sqlsafe}}.reporting_ocp_cost_summary_by_quota_p (
    id,
    cluster_id,
    cluster_alias,
    quota_name,
    ...
    WHERE usage_start >= {{start_date}}::date
        AND usage_start <= {{end_date}}::date
        AND source_uuid = {{source_uuid}}
        AND quota_name IS NOT NULL
    GROUP BY usage_start, cluster_id, cluster_alias, quota_name, quota_type, cost_model_rate_type
;
