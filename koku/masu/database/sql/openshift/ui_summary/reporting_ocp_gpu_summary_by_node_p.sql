DELETE FROM {{schema | sqlsafe}}.reporting_ocp_gpu_summary_by_node_p
WHERE usage_start >= {{start_date}}::date
    AND usage_start <= {{end_date}}::date
    AND source_uuid = {{source_uuid}}
;

INSERT INTO {{schema | sqlsafe}}.reporting_ocp_gpu_summary_by_node_p (
    id,
    cluster_id,
    cluster_alias,
    node,
    ...
    SELECT uuid_generate_v4() as id,
        ...
        max(pod_labels->>'gpu-vendor') as gpu_vendor_name,
        max(pod_labels->>'gpu-model') as gpu_model_name,
        max((pod_labels->>'gpu-memory-mib')::numeric) as gpu_memory_capacity_mib,
