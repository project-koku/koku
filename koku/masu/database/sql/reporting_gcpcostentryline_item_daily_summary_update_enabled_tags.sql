...
cte_filtered_tags AS (
    ...
    FROM (
        SELECT lids.uuid,
            lids.tags as gcp_tags,
            aak.key_array
        FROM {{schema | sqlsafe}}.reporting_gcpcostentrylineitem_daily_summary lids
    ...
    jsonb_each_text(lids.gcp_tags) AS labels
    WHERE key = ANY (key_array)
    GROUP BY lids.uuid
),
cte_joined_tags AS (
    SELECT f.uuid,
        CASE WHEN f.gcp_tags IS NOT NULL
            THEN f.gcp_tags
            ELSE '{}'::jsonb
            END AS tags
    FROM (
        SELECT lids.uuid,
            ft.gcp_tags
        FROM {{schema | sqlsafe}}.reporting_gcpcostentrylineitem_daily_summary AS lids
