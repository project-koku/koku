WITH cte_tag_value AS (
    SELECT key,
        ...
        namespace,
        node
    FROM {{schema_name | sqlsafe}}.reporting_ocpawscostlineitem_project_daily_summary_p AS li,
        jsonb_each_text(li.tags) labels
    WHERE li.usage_start >= {{start_date}}
        AND li.usage_start <= {{end_date}}
        AND li.tags ?| (SELECT array_agg(DISTINCT key) FROM {{schema_name | sqlsafe}}.reporting_awsenabledtagkeys WHERE enabled=true)
    {% if bill_ids %}
...
cte_values_agg AS (
    ...
    JOIN {{schema_name | sqlsafe}}.reporting_awsenabledtagkeys AS etk
        ON tv.key = etk.key
...
cte_distinct_values_agg AS (
    ...
    FROM (
        ...
        LEFT JOIN {{schema_name | sqlsafe}}.reporting_ocpawstags_summary AS ls
            ON va.key = ls.key
...
ins1 AS (
    INSERT INTO {{schema_name | sqlsafe}}.reporting_ocpawstags_summary (uuid, key, values, cost_entry_bill_id, report_period_id, usage_account_id, account_alias_id, namespace, node)
...
INSERT INTO {{schema_name | sqlsafe}}.reporting_ocpawstags_values (uuid, key, value, usage_account_ids, account_aliases, cluster_ids, cluster_aliases, namespaces, nodes)
SELECT uuid_generate_v4() as uuid,
    tv.key,
    tv.value,
...
DELETE FROM {{schema_name | sqlsafe}}.reporting_ocpawstags_summary AS ts
WHERE EXISTS (
    SELECT 1
    FROM {{schema_name | sqlsafe}}.reporting_awsenabledtagkeys AS etk
...
WITH cte_expired_tag_keys AS (
    ...
    FROM {{schema_name | sqlsafe}}.reporting_ocpawstags_values AS tv
    LEFT JOIN {{schema_name | sqlsafe}}.reporting_ocpawstags_summary AS ts
        ON tv.key = ts.key
    WHERE ts.key IS NULL

)
DELETE FROM {{schema_name | sqlsafe}}.reporting_ocpawstags_values tv
