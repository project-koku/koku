...
FROM (
    ...
    FROM (
        SELECT lids.uuid,
            ...
            WHEN {{metric}}='cpu_core_effective_per_hour' THEN greatest(lids.pod_usage_cpu_core_hours, lids.pod_request_cpu_core_hours)
            ...
            WHEN {{metric}}='memory_gb_effective_per_hour' THEN greatest(lids.pod_usage_memory_gigabyte_hours, lids.pod_request_memory_gigabyte_hours)
