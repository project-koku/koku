...
cte_filtered_tags AS (
    ...
    FROM (
        SELECT lids.uuid,
            lids.tags as azure_tags,
            aak.key_array
        FROM {{schema | sqlsafe}}.reporting_azurecostentrylineitem_daily_summary lids
    ...
    jsonb_each_text(lids.azure_tags) AS labels
    WHERE key = ANY (key_array)
    GROUP BY lids.uuid
),
cte_joined_tags AS (
    SELECT f.uuid,
        CASE WHEN f.azure_tags IS NOT NULL
            THEN f.azure_tags
            ELSE '{}'::jsonb
            END AS tags
    FROM (
        SELECT lids.uuid,
            ft.azure_tags
        FROM {{schema | sqlsafe}}.reporting_azurecostentrylineitem_daily_summary AS lids
