...
FROM (
    SELECT
        ...
        usage_start,
        ROW_NUMBER() OVER (
            PARTITION BY pod_labels::json->>'vm_kubevirt_io_name'
    ...
    WHERE
        ...
        AND pod_request_cpu_core_hours IS NOT NULL
        AND pod_labels::json->>'vm_kubevirt_io_name' IS NOT NULL
) AS latest
LEFT JOIN (
    SELECT
        storage.persistentvolumeclaim AS pvc_name,
        pod_usage.pod_labels::json->>'vm_kubevirt_io_name' AS vm_name
    ...
    WHERE storage.persistentvolumeclaim IS NOT NULL
        ...
        AND pod_usage.pod_labels::json->>'vm_kubevirt_io_name' IS NOT NULL
