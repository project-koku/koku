DELETE FROM {{schema | sqlsafe}}.managed_azure_openshift_daily_temp
...
SELECT
    ...
    (azure.quantity * (
        CASE
            WHEN split_part(azure.unitofmeasure, ' ', 1) ~ '^\d+(\.\d+)?$'
    ...
    (SELECT json_object_agg(key, value) FROM json_each_text(azure.tags::json) WHERE key = ANY(etk.enabled_keys))::text as tags, -- Limit tag keys to enabled keys
    ...
    azure.year,
    azure.month,
    EXTRACT(DAY FROM azure.date)::text as day
...
LEFT JOIN cte_agg_tags AS tag_matches
    ON EXISTS (
        ...
        WHERE strpos(tags, matched_tag) != 0
    )
