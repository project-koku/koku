...
with cte_pg_tag_mapping as (
    ...
    FROM (
        ...
        WHERE
            ...
            AND (enabledtagkeys.provider_type = 'AWS' or enabledtagkeys.provider_type = 'OCP')
    ) AS subquery
)
SELECT uuid(),
    ...
    pod_cost,
    project_markup_cost,
    CAST(multimap_from_entries(CAST(
        transform(
            cast(map_entries(cast(json_parse(pod_labels) as map(varchar, varchar))) AS array(row(key varchar, value varchar))),
            entry -> row(
                CASE
                    WHEN contains(map_keys(tag_map), entry.key) AND tag_map[entry.key] IS NOT NULL
                    THEN tag_map[entry.key]
                    ELSE entry.key
                END,
                entry.value
            )
    ...
    CAST(multimap_from_entries(CAST(
        transform(
            cast(map_entries(cast(json_parse(tags) as map(varchar, varchar))) AS array(row(key varchar, value varchar))),
            entry -> row(
                CASE
                    WHEN contains(map_keys(tag_map), entry.key) AND tag_map[entry.key] IS NOT NULL
                    THEN tag_map[entry.key]
...
-- Note: This change would store the tags in a key value pairs where the values become a list
-- Example:
-- {
--     "qa": ["approved"],
--     "app": ["cost"],
...
-- Scenario
-- I have mapped dashed-key-on-aws to the parent key of app.
-- The catch; no duplicate keys are allowed in the JSON.
