...
-- Node Pod Directly resource_id matching
...
WITH cte_ocp_filtered_resources as (
    SELECT
        distinct azure_tmp.resource_id as azure_partial_resource_id
    FROM hive.{{schema | sqlsafe}}.reporting_ocpusagelineitem_daily_summary as ocp
    JOIN hive.{{schema | sqlsafe}}.azure_openshift_daily_resource_matched_temp as azure_tmp
        ON (azure_tmp.usage_start = ocp.usage_start)
        AND (strpos(azure_tmp.resource_id, ocp.persistentvolume) > 0 AND ocp.data_source = 'Storage')
    WHERE ocp.source = {{ocp_source_uuid}}
        AND ocp.year = {{year}}
        ...
        AND ocp.usage_start < date_add('day', 1, {{end_date}})
        AND (ocp.resource_id IS NOT NULL AND ocp.resource_id != '')
        AND azure_tmp.ocp_source = {{ocp_source_uuid}}
        AND azure_tmp.year = {{year}}
        AND azure_tmp.month = {{month}}
),
...
FROM (
    ...
    JOIN hive.{{schema | sqlsafe}}.azure_openshift_daily_resource_matched_temp as azure
        ON (azure.usage_start = ocp.usage_start)
        AND (strpos(azure.resource_id, ocp.persistentvolume) > 0 AND ocp.data_source = 'Storage')
    JOIN cte_az_resource_to_disk_capacity AS az_disk
