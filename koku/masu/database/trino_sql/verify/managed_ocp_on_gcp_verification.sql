...
(
    ...
    FROM hive.{{schema | sqlsafe}}.reporting_ocpgcpcostlineitem_project_daily_summary as parquet_table
    WHERE parquet_table.gcp_source = {{cloud_source_uuid}}
    AND parquet_table.year = {{year}}
    AND lpad(parquet_table.month, 2, '0') = {{month}}
    AND namespace not in ('Worker unallocated', 'Platform unallocated')
) t2;
