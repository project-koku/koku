...
class AWSReportDBAccessorTest(MasuTestCase):
    ...
    def test_delete_ocp_on_aws_hive_partition_by_day(self, mock_execute_trino_with_description, mock_table_exist):
        ...
        with patch("masu.database.aws_report_db_accessor.AWSReportDBAccessor.schema_exists_trino", return_value=True):
            try:
                self.delete_ocp_on_aws_hive_partition_by_day([1], self.ocp_provider_uuid, "2022", "01")
        ...
        with patch("masu.database.aws_report_db_accessor.AWSReportDBAccessor.schema_exists_trino", return_value=False):
            self.delete_ocp_on_aws_hive_partition_by_day([1], self.ocp_provider_uuid, "2022", "01")
