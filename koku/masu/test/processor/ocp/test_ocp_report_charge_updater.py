...
#             items = (
#                 self.accessor._get_db_obj_query(table_name).filter(
#                   report_period=usage_period, data_source="Pod").all()
#             )
#             for item in items:
...
#                     mem_usage_value * mem_rate_value, Decimal(item.pod_charge_memory_gigabyte_hours), places=6
#                 )
...
#         rate_metric_map = {"storage_gb_usage_per_month": usage_rate, "storage_gb_request_per_month": request_rate}
#         mock_markup.return_value = markup
#         mock_rate_map.return_value = rate_metric_map
...
#         rate = [{"metric": {"name": "memory_gb_usage_per_hour"}, "tiered_rates": [{"value": 1, "unit": "USD"}]}]
...
#         rate = [{"metric": {"name": "cpu_core_usage_per_hour"}, "tiered_rates": [{"value": 1.5, "unit": "USD"}]}]
...
#                     derived_cost += (
#                         item.pod_charge_memory_gigabyte_hours if item.pod_charge_memory_gigabyte_hours else Decimal(0)
#                     )
#                     derived_cost += (
...
#                         derived_cost * openshift_markup["value"] / 100 + infra_cost * aws_markup["value"] / 100
#                     )
#                     expected_project_markup_cost = (
#                         derived_cost * openshift_markup["value"] /
#                           100 + project_infra_cost * aws_markup["value"] / 100
#                     )
...
#             {"metric": {"name": "cpu_core_usage_per_hour"}, "tiered_rates": [{"value": 2.5, "unit": "USD"}]}
#         ]
...
#         )
#         table_name = OCP_REPORT_TABLE_MAP["line_item_daily_summary"]
#         items = self.accessor._get_db_obj_query(table_name).filter(
#               report_period=usage_period, data_source="Pod").all()
#         with schema_context(self.schema):
#             for item in items:
#                 cpu_usage_value = (
#                     Decimal(item.pod_usage_cpu_core_hours) if item.pod_usage_cpu_core_hours else Decimal(0.0)
#                 )
#                 self.assertAlmostEqual(0.0, Decimal(item.pod_charge_memory_gigabyte_hours), places=5)
#                 self.assertAlmostEqual(
#                     cpu_usage_value * cpu_rate_value, Decimal(item.pod_charge_cpu_core_hours), places=6
#                 )
#     def test_update_monthly_cost(self):
#         """Test OCP charge for monthly costs is updated."""
#         node_cost = random.randrange(1, 200)
#         monthly_cost_metric = [
#             {"metric": {"name": "node_cost_per_month"}, "tiered_rates": [{"value": node_cost, "unit": "USD"}]}
#         ]
...
#         )
#         with schema_context(self.schema):
#             monthly_cost_row = OCPUsageLineItemDailySummary.objects.filter(monthly_cost__isnull=False).first()
