...
class OCPCloudReportSummaryUpdaterTest(MasuTestCase):
    ...
    def _generate_ocp_on_aws_data(self, cluster_id=None):
        ...
        for cost_entry_date in (today, last_month):
            ...
            creator.create_ocp_node_label_line_item(period, report)
    ...
    def _generate_ocp_on_azure_data(self, cluster_id=None):
        ...
        with schema_context(self.schema):
            for cost_entry_date in (today, last_month):
                bill = creator.create_azure_cost_entry_bill(
                    provider_uuid=self.azure_provider.uuid, bill_date=cost_entry_date
                )
                bill_ids.append(str(bill.id))
        ...
        with OCPReportDBAccessor(self.schema, self.column_map) as ocp_accessor:
            for cost_entry_date in (today, last_month):
                period = creator.create_ocp_report_period(
                    self.ocp_test_provider_uuid, period_date=cost_entry_date, cluster_id=cluster_id
                )
                report = creator.create_ocp_report(period, cost_entry_date)
                creator.create_ocp_usage_line_item(period, report, node=node)
                creator.create_ocp_node_label_line_item(period, report, node=node)
    ...
    def test_get_infra_map(self):
        ...
        updater = OCPCloudReportSummaryUpdater(schema=self.schema, provider=ocp_provider, manifest=None)
        ...
        updater = OCPCloudReportSummaryUpdater(schema=self.schema, provider=aws_provider, manifest=None)
    ...
    def test_update_summary_tables_azure(self, mock_markup, mock_refresh):
        ...
        self.assertAlmostEqual(markup_cost, pretax_cost * decimal.Decimal(markup.get("value") / 100), places=9)
    ...
    def test_refresh_openshift_on_infrastructure_views(self):
        ...
        updater = OCPCloudReportSummaryUpdater(schema=self.schema, provider=provider, manifest=None)
