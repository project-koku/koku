...
class OCIReportDownloaderTest(MasuTestCase):
    ...
    def create_oci_downloader_with_mocked_values(
        ...
        """
        Create a OCIReportDownloader that skips the initial OCI bigquery check creates etag.
        ...
        Returns:
            OCIReportDownloader instance with faked argument data and Mocks in
            self.etag.
        ...
        with patch("masu.external.downloader.oci.oci_report_downloader.OCIProvider"), patch(
            "masu.external.downloader.oci.oci_report_downloader.OCIReportDownloader._generate_etag",
    ...
    def test_generate_etag_big_query_client_error(self, oci_provider):
        ...
        err_msg = "OCI Error"
        with patch("masu.external.downloader.oci.oci_report_downloader.bigquery") as bigquery:
    ...
    @patch("masu.external.downloader.oci.oci_report_downloader.OCIProvider")
    def test_generate_etag(self, oci_provider):
        """Test BigQuery client is handled correctly in generate etag method."""
        billing_source = {"table_id": FAKE.slug(), "dataset": FAKE.slug()}
        credentials = {"project_id": FAKE.slug()}
        with patch("masu.external.downloader.oci.oci_report_downloader.bigquery") as bigquery:
    ...
    @patch("masu.external.downloader.oci.oci_report_downloader.bigquery")
    def test_download_file_failure_on_file_open(self, mock_bigquery, mock_makedirs):
        ...
        with patch("masu.external.downloader.oci.oci_report_downloader.open") as mock_open:
            err_msg = "bad open"
    ...
    def test_download_with_unreachable_source(self, oci_provider):
        ...
        with self.assertRaises(OCIReportDownloaderError):
            OCIReportDownloader(FAKE.name(), billing_source, credentials=credentials)
    ...
    def test_get_dataset_name(self):
        ...
        for dataset in datasets:
            ...
            with patch("masu.external.downloader.oci.oci_report_downloader.OCIProvider"), patch(
                ...
                with patch("masu.external.downloader.oci.oci_report_downloader.OCIProvider"):
                    downloader = OCIReportDownloader(
                        customer_name=FAKE.name(),
                        data_source=billing_source,
                        provider_uuid=uuid4(),
