...
class ApplicationsView(APIView):
    ...
    Since Cost Management is the only supported application (application_type_id=COST_MGMT_APP_TYPE_ID),
    ...
    def get(self, request):
        ...
        # Support filter[application_type_id] - only COST_MGMT_APP_TYPE_ID is valid
        filter_app_type_id = request.query_params.get("filter[application_type_id]")
        if filter_app_type_id and filter_app_type_id != COST_MGMT_APP_TYPE_ID:
            # No sources match non-cost-management application_type_id
    ...
    def post(self, request):
        ...
        if application_type_id and application_type_id != COST_MGMT_APP_TYPE_ID:
            return Response(
                {"error": f"Only application_type_id '{COST_MGMT_APP_TYPE_ID}' (cost-management) is supported"},
                status=status.HTTP_400_BAD_REQUEST,
            )
