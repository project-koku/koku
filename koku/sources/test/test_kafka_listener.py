...
class SourcesKafkaMsgHandlerTest(IamTestCase):
    ...
    def setUp(self):
        ...
        self.mock_create_requests = {
            Provider.PROVIDER_AWS: [
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_APPLICATION_TYPES}/{COST_MGMT_APP_TYPE_ID}/sources?filter[id]={self.source_ids.get(Provider.PROVIDER_AWS)}",  # noqa: E501
                    "status": 200,
                    "json": {"data": [{"not": "empty"}]},
                },
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_SOURCE_TYPES}?filter[id]={SOURCE_TYPE_IDS_MAP.get(Provider.PROVIDER_AWS)}",  # noqa: E501
                    "status": 200,
                    "json": {"data": [{"name": SOURCE_TYPE_IDS[SOURCE_TYPE_IDS_MAP[Provider.PROVIDER_AWS]]}]},
                },
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_APPLICATIONS}?filter[source_id]={self.source_ids.get(Provider.PROVIDER_AWS)}",  # noqa: E501
                    "status": 200,
                    "json": {"data": [{"extra": {"bucket": "test_bucket"}}]},
                },
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_AUTHENTICATIONS}?filter[source_id]={self.source_ids.get(Provider.PROVIDER_AWS)}",  # noqa: E501
                    "status": 200,
                    "json": {"data": [{"id": self.source_ids.get(Provider.PROVIDER_AWS), "username": FAKE_AWS_ARN}]},
                },
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_SOURCES}/{self.source_ids.get(Provider.PROVIDER_AWS)}",  # noqa: E501
                    "status": 200,
                    "json": {
                        "name": "Provider AWS",
            ...
            Provider.PROVIDER_OCP: [
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_APPLICATION_TYPES}/{COST_MGMT_APP_TYPE_ID}/sources?filter[id]={self.source_ids.get(Provider.PROVIDER_OCP)}",  # noqa: E501
                    "status": 200,
                    "json": {"data": [{"not": "empty"}]},
                },
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_SOURCE_TYPES}?filter[id]={SOURCE_TYPE_IDS_MAP.get(Provider.PROVIDER_OCP)}",  # noqa: E501
                    "status": 200,
                    "json": {"data": [{"name": SOURCE_TYPE_IDS[SOURCE_TYPE_IDS_MAP[Provider.PROVIDER_OCP]]}]},
                },
                {
                    "url": f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_SOURCES}/{self.source_ids.get(Provider.PROVIDER_OCP)}",  # noqa: E501
                    "status": 200,
                    "json": {
                        "name": "Provider OCP",
    ...
    def test_message_not_associated_with_cost_mgmt(self):
        ...
        for test in table:
            with self.subTest(test=test):
                with requests_mock.mock() as m:
                    m.get(
                        url=f"{MOCK_URL}/api/sources/v1.0/{ENDPOINT_APPLICATION_TYPES}/{COST_MGMT_APP_TYPE_ID}/sources?filter[id]=1",  # noqa: E501
                        status_code=200,
                        json={"data": []},
                    )
