...
class GunicornConfigTest(TestCase):
    ...
    @patch("gunicorn_conf.faulthandler.enable")
    def test_post_fork_enables_faulthandler(self, mock_enable, mock_unleash):
        worker = Mock(pid=123)

        gunicorn_conf.post_fork(Mock(), worker)

        mock_enable.assert_called_once_with(file=sys.stderr, all_threads=True)
        mock_unleash.initialize_client.assert_called_once_with()

    @patch("gunicorn_conf.threading.get_ident", return_value=123)
    ...
    def test_worker_abort_reports_all_thread_stacks(
        self, mock_sentry_enabled, mock_current_frames, mock_threads, mock_get_ident
    ):
        ...
        self.assertEqual(event["message"]["formatted"], "Gunicorn worker timeout (pid:456)")
        self.assertEqual(event["threads"]["values"][0]["name"], "gunicorn-worker")
        self.assertTrue(event["threads"]["values"][0]["current"])
        self.assertTrue(event["threads"]["values"][0]["stacktrace"]["frames"])
        sentry_sdk.flush.assert_called_once_with(timeout=1)
        self.assertIn("Thread gunicorn-worker (id: 123):", worker.log.error.call_args.args[0])
    ...
    def test_worker_abort_skips_sentry_when_disabled(self, mock_sentry_enabled):
        ...
        worker.log.error.assert_called_once()
