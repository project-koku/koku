...
CREATE MATERIALIZED VIEW reporting_ocp_volume_summary AS(
    SELECT row_number() OVER(ORDER BY usage_start, cluster_id, cluster_alias, currency) as id,
    ...
    GROUP BY usage_start, cluster_id, cluster_alias, source_uuid, currency
)
WITH DATA
;

CREATE UNIQUE INDEX ocp_volume_summary
ON reporting_ocp_volume_summary (usage_start, cluster_id, cluster_alias, source_uuid, currency)
;
