...
def create_ocpall_partitions(apps, schema_editor):
    ...
    sql_stmts = [
        [
            f"Creating default partition for {p_table_name}",
            (
                """
INSERT
...
SELECT current_schema,
       %s::text || (to_char(part_start, '_YYYY_MM'))::text,
       %s::text,
       'range',
       'usage_start',
       jsonb_build_object(
           'default', false,
           'from', to_char(part_start, 'YYYY-MM-01'),
           'to', to_char((part_start + '1 month'::interval), 'YYYY-MM-01')
       ),
       true
...
def drop_ocpall_partitions(apps, schema_editor):
    ...
    sql_stmts = [
        [
            f"Dropping partitions of {p_table_name}",
            (
                """
DELETE
...
def create_new_ocpall_matviews(apps, schema_editor):
    ...
    for view in ocpallviews:
        ...
        view_sql = view_sql.replace(view, new_view)
        LOG.info(f"Create materialized view {new_view}")
        with conn.cursor() as cur:
...
def drop_new_ocpall_matviews(apps, schema_editor):
    ...
    for view in ocpallviews:
        new_view = f"{view}_p"
        LOG.info(f"Drop materialized view {new_view}")
        with conn.cursor() as cur:
            cur.execute(f"DROP MATERIALIZED VIEW IF EXISTS {new_view} ;")
