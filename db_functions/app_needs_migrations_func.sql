...
BEGIN
    ...
    LOOP
        ...
        THEN
            ...
            THEN
                RAISE WARNING '    Tenant % exists, but there is no database schema. Skipping tenant %.',
                            schema_rec.schema_name,
                            schema_rec.schema_name;
                RAISE LOG 'MIGRATION CHECK :: Tenant % exists, but there is no database schema.',
                            schema_rec.schema_name;
            END IF;
            IF NOT exists_rec.tenant_exists AND exists_rec.schema_exists
            THEN
                RAISE WARNING '    Schema % exists, but there is no tenant record. Skipping schema %.',
                            schema_rec.schema_name,
                            schema_rec.schema_name;
                RAISE LOG 'MIGRATION CHECK :: Schema % exists, but there is no tenant record.',
                            schema_rec.schema_name;
            END IF;
        END IF;
