...
function _set_IQE_filter_expressions_for_smoke_labels() {
    ...
    if grep -E "aws-smoke-tests" <<< "$SMOKE_LABELS"; then
        ...
        export IQE_FILTER_EXPRESSION="(test_api_ocp or test_api_cost_model_ocp) and not ocp_on_gcp and not ocp_on_azure and not ocp_on_cloud"
        export IQE_MARKER_EXPRESSION="cost_required and not outage"
